...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        foreach ($reminders as $reminder) {
            // Convert time to full datetime if needed
            $time = $reminder->time;
            if (strlen($time) <= 5) {
                $time = date('Y-m-d H:i:s', strtotime($reminder->date . ' ' . $time));
            }
    ...
    {
        // Create new table with original column types
        ...
        foreach ($reminders as $reminder) {
            DB::table('reminders_old')->insert([
                ...
                'description' => $reminder->description,
                'date' => $reminder->date,
                'time' => date('H:i:s', strtotime($reminder->time)),
                'created_at' => $reminder->created_at,
                'updated_at' => $reminder->updated_at,
            ]);
        }
